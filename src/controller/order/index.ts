...
JOIN 
  product 
ON 
  orders.productId = product.product_id 
WHERE 
  ${where} = $1;`;
};
...
const getAllOrders = async (req: Request, res: Response): Promise<any> => {
  try {
    const orders = await client.query(
      "SELECT orders.qyt,orders.status, orders.seller_id, orders.orderdate,product.id AS id,product.product_id,product.name,product.rate, product.photo,product.size FROM orders JOIN product ON orders.productId = product.product_id "
    );
